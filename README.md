...
しかし、実際には平均値や上下限値で設計が成立しない場合が多く、演算結果の分布を正確に求めてバランスを設計する必要があるにもかかわらず、
...
- 市場走行データを活用した設計方法,Toyota Technical Review 2018/5 Vol64.p95
  ...
  - アクチュエータの走行距離あたりの作動頻度分布(回/km)と生涯走行距離分布(km)の積から生涯作動回数のストレスストレングス分布(回)を作成して、耐久条件を求める.
...
- ビックデータを活用した制御リスク設計,自動車技術会20年秋季大会学術講演会
  ...
  - レーダー認識位置分布(m)と相対速度(m/sec)と車線変更制御完了時間(sec)から安全な車線変更制御設計を行う.
